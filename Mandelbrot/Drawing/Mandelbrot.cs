...
namespace Drawing
{
    ...
    public partial class Mandelbrot : Form
    {
                ...
                for (var y = _yMin; y < _yMax; y += xyStep.Imaginary)
                {
                    ...
                    for (var x = _xMin; x < _xMax; x += xyStep.Real)
                    {
                        ...
                        if (k < iterations)
                        {
                            ...
                            {
                                _myBitmap.SetPixel(xPix, yPix, _colourTable.GetColour(k));
                            }
                        }
        ...
        private void MouseUpOnForm(object sender, MouseEventArgs e)
        {
            ...
            var pixelCoord = new ComplexPoint((int)(_xValue - 1005d / zoomScale / 4), (int)(_yValue - 691d / zoomScale / 4));
            _zoomCoord1 = _myPixelManager.GetAbsoluteMathsCoord(pixelCoord);
            var pixelCoord2 = new ComplexPoint((int)(_xValue + 1005d / zoomScale / 4), (int)(_yValue + 691d / zoomScale / 4));
            _zoomCoord2 = _myPixelManager.GetAbsoluteMathsCoord(pixelCoord2);
            ...
            yMinCheckBox.Text = _zoomCoord1.Imaginary.ToString(CultureInfo.InvariantCulture);
            yMaxCheckBox.Text = _zoomCoord2.Imaginary.ToString(CultureInfo.InvariantCulture);
            xMinCheckBox.Text = _zoomCoord1.Real.ToString(CultureInfo.InvariantCulture);
            xMaxCheckBox.Text = _zoomCoord2.Real.ToString(CultureInfo.InvariantCulture);
